...
export default function AttendanceSummary({ students = [] }) {
  ...
  const { presentCount, absentCount, totalStudents, attendanceRate } = useMemo(() => {
  ...
  return (
    <div>
      ...
      <div style={{ padding: '10px', border: '1px solid #ccc', borderRadius: '8px', marginBottom: '20px', textAlign: 'center' }}>
        <h4>Evaluation Report</h4>
        <p><strong>Total Students:</strong> {totalStudents}</p>
        <p><strong>Present:</strong> {presentCount}</p>
        <p><strong>Absent:</strong> {absentCount}</p>
        <p><strong>Attendance Rate:</strong> <span style={{ color: attendanceRate >= 50 ? '#00C49F' : '#FF8042', fontWeight: 'bold' }}>{attendanceRate}%</span></p>
      </div>
